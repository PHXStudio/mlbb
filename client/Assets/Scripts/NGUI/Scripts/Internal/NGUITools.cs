...
static public class NGUITools
{
	...
	{
#if UNITY_FLASH
		ClientLog.Instance.LogError("WWW is not yet implemented in Flash");
		return null;
#else
		WWW www = null;
		try { www = new WWW(url); }
		catch (System.Exception ex) { ClientLog.Instance.LogError(ex.Message); }
		return www;
#endif
	}
	...
	static public WWW OpenURL (string url, WWWForm form)
	{
		...
		catch (System.Exception ex) { ClientLog.Instance.LogError(ex != null ? ex.Message : "<null>"); }
		return www;
#endif
	}
	...
	static public void SetDirty (UnityEngine.Object obj)
	{
		...
		if (obj)
		{
			//if (obj is Component) ClientLog.Instance.Log(NGUITools.GetHierarchy((obj as Component).gameObject), obj);
			//else if (obj is GameObject) ClientLog.Instance.Log(NGUITools.GetHierarchy(obj as GameObject), obj);
			//else ClientLog.Instance.Log("Hmm... " + obj.GetType(), obj);
	...
	static public bool Save (string fileName, byte[] bytes)
	{
		...
		{
			ClientLog.Instance.Log(ex.Message);
			return false;
		}
