...
public class UIWidget : UIRect
{
	...
	public void CheckLayer ()
	{
		...
		{
			ClientLog.Instance.LogWarning("You can't place widgets on a layer different than the UIPanel that manages them.\n" +
	...
	protected override void OnStart ()
	{
		...
		{
			ClientLog.Instance.LogError("Widgets and panels should not be on the same object! Widget must be a child of the panel.", this);
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			ClientLog.Instance.LogError("You should not place more than one widget on the same object. Weird stuff will happen!", this);
		}
#endif
		CreatePanel();
	}
		...
		if (mChanged)
		{
			...
			if (mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			{
				...
				if (geometry.hasVertices)
				{
					...
					//ClientLog.Instance.Log("Fill " + name + " (" + Time.time + ")");
