        ...
        private void GetResponse() {
            ...
            try {
                ...
                while (++retry < maximumRetryCount) {
                    ...
                    string newServerIp = "";
                    string serip = uri.Host;
                    AddressFamily newAddressFamily = AddressFamily.InterNetwork;
                    NetConnection.Instance.getIPType(uri.Host, uri.Port.ToString(), out newServerIp, out newAddressFamily);
                    if (!string.IsNullOrEmpty(newServerIp)) { serip = newServerIp; }

                    var client = new TcpClient(newAddressFamily);
                    client.Connect(serip, uri.Port);
