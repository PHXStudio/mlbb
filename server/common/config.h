...
#define ACE_REACTOR_MAKE                                                       \
  do {                                                                         \
    static ACE_Select_Reactor __select_reactor;                                \
    static ACE_Reactor __reactor(&__select_reactor);                           \
    ACE_Reactor::instance(&__reactor);                                         \
  } while (0)
#else
#define ACE_REACTOR_MAKE                                                       \
  do {                                                                         \
...
#define SET_BIT_BOOL(FLAG, OFFSET, B)                                          \
  FLAG = B ? (FLAG | (1 << OFFSET)) : (FLAG & (~(1 << OFFSET)))
#define COMPARE_BIT_BOOL(FLAG0, FLAG1, OFFSET)                                 \
  (GET_BIT_BOOL(FLAG0, OFFSET) == GET_BIT_BOOL(FLAG1, OFFSET))
...
#define XML_GET_ATTRIBUTE_B(Element, AttributeName, Target_Bool)               \
  do {                                                                         \
    const char *tmp = Element->Attribute(AttributeName);                       \
    if (tmp)                                                                   \
      Target_Bool = (ACE_OS::strcasecmp(tmp, "true") ? false : true);          \
  } while (0)
...
#define XML_GET_S(Node, Target_Str)                                            \
  do {                                                                         \
    TiXmlText *text = Node->ToText();                                          \
    if (text)                                                                  \
      Target_Str = text->Value();                                              \
  } while (0)
...
std::string StrLocalTime(uint64 t64,
...
std::vector<std::string> Split(std::string const &str,
                               std::string const &delims, U32 maxSplits = 0);
void Split(const char *line, char delims, std::vector<std::string> &out,
           bool skip = true);
...
template <class T> inline std::string ToString(T const &v) {
  std::stringstream ss;
  ss << v;
  return ss.str();
}
template <class T> inline T Convert(std::string const &v) {
  std::stringstream ss;
  ss << v;
  T r;
  ss >> r;
...
inline bool IsInRange(const COM_FPosition &p0, const COM_FPosition &p1,
                      const float r) {
  return Length(Sub(p0, p1)) <= r;
}

#define SINGLETON_FUNCTION(T)                                                  \
  static T *instance() {                                                       \
    static T T##INST;                                                          \
    return (&T##INST);                                                         \
  }
...
struct RuntimeProbe {
  ...
  inline float begin() { return start_ = ACE_OS::gettimeofday().sec(); }

  inline float end() { return stop_ = ACE_OS::gettimeofday().sec(); }
...
struct FunctionProbe : RuntimeProbe {
  FunctionProbe(const char *filename, const char *funcname, int line)
      : filename_(filename), funcname_(funcname), line_(line) {
    begin();
  }
  ~FunctionProbe() {
    end();
    ACE_DEBUG((LM_INFO, "Function probe:%s(%s:%d) runtime %f\n", funcname_,
               filename_, line_, interval()));
  }
...
struct ChunkProbe : RuntimeProbe {
  ChunkProbe(const char *filename) : filename_(filename) {}
...
typedef std::unordered_map<std::string, SGE_ContactInfoExt *>
    NameContentInfoMap;
typedef std::unordered_map<U32, SGE_ContactInfoExt *> IdContentInfoMap;
typedef std::unordered_map<int32, SGE_PlayerEmployeeQuest *>
    PlayerEmployeeQuestTable;
#else
typedef std::unordered_map<std::string, Player *> NamePlayerMap;
typedef std::unordered_map<U32, Player *> IdPlayerMap;
typedef std::unordered_map<S32, Guild *> IdGuildMap;
...
#define FUNCTION_PROBE                                                         \
  FunctionProbe __FUNCTION_RUNTIME_PROBE__(__FILE__, __FUNCTION__, __LINE__);
#define CHUNK_PROBE_BEGIN(NAME)                                                \
  ChunkProbe __CHUNK_PROBE_##NAME##__(__FILE__);                               \
  __CHUNK_PROBE_##NAME##__._begin(__LINE__);
#define CHUNK_PROBE_END(NAME) __CHUNK_PROBE_##NAME##__._end();

#define NEW_MEM(T, ...)                                                        \
  new T(                                                                       \
      __VA_ARGS__); // new(MemoryLeaks::Ref().Malloc(sizeof(T),__FILE__,__LINE__))T(__VA_ARGS__);
